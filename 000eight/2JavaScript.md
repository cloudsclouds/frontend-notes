# JavaScript
## 1. JS 事件循环
JS 事件循环是单线程运行时的任务调度机制，分同步任务和异步任务。同步任务直接在主线程执行，异步任务会进入任务队列。主线程执行完同步任务后，会从任务队列取异步任务执行，异步任务又分宏任务和微任务，微任务优先级更高，会先清空微任务队列，再执行一个宏任务，如此循环。常见微任务有 Promise.then、async/await，宏任务有 setTimeout、DOM 事件、AJAX。

## 1.1 大数据渲染是如何基于事件循环优化的？
核心是把大量 DOM 操作拆分成微任务或宏任务，避免阻塞主线程。比如用 requestAnimationFrame 在重绘前更新 DOM，每次只处理一部分数据，让主线程有空闲响应用户操作。

也可以用虚拟列表，只渲染可视区域内的 DOM，通过监听滚动动态更新显示内容，大幅减少 DOM 数量。

还能结合 Web Worker 在后台处理数据计算，再把结果通过 postMessage 传给主线程渲染，不占用主线程资源。

## 2. webworker 优势 如何新建和通信 劣势
