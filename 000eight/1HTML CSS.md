...
如果面试官更细抠，还可以补充绝对定位移出视口、`z-index` 遮挡等方式。

---

## 21. `z-index` 为什么有时候不生效？
...
所以 `z-index` 问题很多时候不是子元素本身的问题，而是父级堆叠上下文的问题。
...
## 23. `requestAnimationFrame` 和 `setTimeout` 有什么区别？
...
一句话总结：
- 动画优先用 `requestAnimationFrame`
- 普通延时逻辑才考虑 `setTimeout`

---

## 24. 常见 CSS 单位有哪些？`px`、`em`、`rem`、`vw/vh` 的区别？
...
## 25. CSS 可继承属性和不可继承属性有哪些？

### 面试逐字稿

一般来说，文本相关属性比较容易继承，比如：
- `color`
- `font-size`
- `font-family`
- `line-height`

而盒模型、布局、定位相关通常不继承，比如：
- `width`
- `height`
- `margin`
- `padding`
...
面试里不用背得特别死，但要知道“文本类大多可继承，布局类大多不可继承”。
...
## 31. `transition` 和 `animation` 有什么区别？
...
一句话总结：
- `transition`：A 到 B 的变化
- `animation`：自己定义完整运动过程
...
“这个问题我一般会从概念、适用场景、优缺点和工程实践四个角度回答。如果是 HTML/CSS 题，我还会补一个最常见的代码实现，这样回答会更完整。”
